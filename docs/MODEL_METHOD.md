...
| --- | --- | --- |
| Elo rating | `(eloA - eloB) x w.elo` | `elo = 1.0` |
| FIFA ranking | `(rankB - rankA) x w.fifaRankingPerPlace`, capped | `1.4` / cap `90` |
| Squad quality | `(sqA - sqB) x w.squadQuality` | `4.0` |
| Recent form | `(formA - formB) x w.recentForm` | `2.0` |
| Manager cohesion | `(sameA - sameB) x w.manager` | `15` |
| Host advantage | `(hostA - hostB) x w.host` | `60` |
...
`structuralDepth  in  [0,1]` blends **log-scaled GDP per capita** (footballing
...
lambda. We enumerate the joint matrix up to 8 goals/side (upper tail folded into the
last bucket so it sums to ~1), then:
- **Win/draw/loss** = summing the appropriate matrix cells.
- **Top scorelines** = the most probable cells.

This module knows nothing about teams - only lambda values - so it can be replaced by
