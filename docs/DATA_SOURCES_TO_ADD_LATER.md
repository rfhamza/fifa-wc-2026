...
| Model features (Elo, GDP, population, squad, form, climate, FIFA ranking values) | **manual / placeholder (Phase 1.7 layer)** | Now flow through the versioned model-input layer `data/model-inputs/` with honest per-family status. Elo/FIFA ranking/structural = `manual`; squad/form/climate = `placeholder` (weight-capped so they can't silently drive probabilities). Nothing claimed source-backed. Supply an authoritative snapshot (e.g. FIFA ranking, eloratings, World Bank) to promote a family via `lib/data/validate-model-inputs.ts`. |
