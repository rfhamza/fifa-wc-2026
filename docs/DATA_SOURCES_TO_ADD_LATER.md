...
| Model features (Elo, GDP, population, squad, form, climate, FIFA ranking values) | **mixed (Phase 1.7/1.8/1.10/1.12 layer)** | Flow through the versioned model-input layer `data/model-inputs/` with honest per-family status. **`fifaRanking` is `source-backed`** from a supplied FIFA snapshot (Phase 1.8, see `docs/FIFA_RANKING_SNAPSHOT_AUDIT.md`) and **`eloRating` is `source-backed`** from the supplied 11 Jun 2026 World Football Elo snapshot (Phase 1.10, see `docs/ELO_RATING_SNAPSHOT_AUDIT.md`). **Structural (GDP + population) is now `candidate` (mixed)** - 46 economies `source-backed` from the World Bank WDI 2024, England/Scotland `manual` (Phase 1.12, see `docs/STRUCTURAL_ECONOMIC_SNAPSHOT_AUDIT.md`). Squad/form/climate = `placeholder` (weight-capped). Supply an authoritative snapshot (market value, results feed) to promote further families via `lib/data/validate-model-inputs.ts`. |
...
| `gdpPerCapita`, `population` | **`candidate` (mixed, Phase 1.12)** | World Bank WDI (`NY.GDP.PCAP.CD`, `SP.POP.TOTL`, `NY.GDP.MKTP.CD`) | 2024; 46 economies source-backed, England/Scotland still hand-authored (`manual`, no separate WB economy). Promote those via ONS / Scottish Government. See `docs/STRUCTURAL_ECONOMIC_SNAPSHOT_AUDIT.md`. |
