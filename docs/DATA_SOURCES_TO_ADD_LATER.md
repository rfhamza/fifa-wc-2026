...
| Round-of-32 skeleton (M73-M104) + Annexe C | **VERIFIED (active)** | Transcribed from the official FWC26 regulations PDF and verified 2026-06-17; `data/official/bracket.ts` is `sourceStatus: "verified"` (full M73-M104 graph + all 495 Annexe C rows), `validateBracket` passes, and the official path is active in production. PDF binary NOT committed. See `docs/OFFICIAL_BRACKET_TRANSCRIPTION_AUDIT.md`. (`seedBracket()` remains only as an inactive fallback.) |
