...
| Model features (Elo, GDP, population, squad, form, climate, FIFA ranking values) | **mixed (Phase 1.7/1.8/1.10 layer)** | Flow through the versioned model-input layer `data/model-inputs/` with honest per-family status. **`fifaRanking` is `source-backed`** from a supplied FIFA snapshot (Phase 1.8, see `docs/FIFA_RANKING_SNAPSHOT_AUDIT.md`) and **`eloRating` is now `source-backed`** from the supplied 11 Jun 2026 World Football Elo snapshot (Phase 1.10, see `docs/ELO_RATING_SNAPSHOT_AUDIT.md`). Structural = `manual`; squad/form/climate = `placeholder` (weight-capped). Supply an authoritative snapshot (World Bank, market value) to promote further families via `lib/data/validate-model-inputs.ts`. |
