...
| Model features (Elo, GDP, population, squad, form, climate, FIFA ranking values) | **mixed (Phase 1.7/1.8 layer)** | Flow through the versioned model-input layer `data/model-inputs/` with honest per-family status. **`fifaRanking` is now `source-backed`** from a supplied FIFA snapshot (Phase 1.8, see `docs/FIFA_RANKING_SNAPSHOT_AUDIT.md`). Elo/structural = `manual`; squad/form/climate = `placeholder` (weight-capped). Supply an authoritative snapshot (eloratings, World Bank) to promote further families via `lib/data/validate-model-inputs.ts`. |
