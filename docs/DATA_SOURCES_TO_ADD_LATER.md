...
| Official schedule + draw positions (Phase 1.6 Step A) | **official source, staged - pending activation** | Transcribed from the OFFICIAL FIFA PDF (`FWC26_Match_Schedule_v17_10042026_EN.pdf`, v17 / 10 Apr 2026, ET, **subject to change**) into `data/official/staging/`: 72 fixtures (ET + UTC, venues, match #s) + all 48 solved draw positions. Validated + dry-run-proven to pass activation; 72/72 cross-check vs candidate. NOT yet activated - `fixtureSource` stays `position-generated` until Step B is approved. PDF binary NOT committed. See `docs/OFFICIAL_SCHEDULE_TRANSCRIPTION_AUDIT.md`. |
