...
export const INPUT_REGISTRY: readonly InputRegistryEntry[] = [
  ...
  },
  {
    ...
    family: "squad / player",
    description:
      "Binary same-nationality-manager cohesion proxy, derived from cross-verified identity data. Wired into the driver pipeline but currently DISABLED (zero model weight) pending an out-of-sample backtest, so it does not move probabilities today.",
    status: "implemented",
    phase: "preTournament",
    ...
    tuningEligible: false,
    liveEligible: false,
    knownLimitations: "Crude binary proxy; not a measure of actual cohesion; confounded with strength already captured by Elo/FIFA. Production weight is currently ZERO (disabled pending backtest).",
    testsRequired: "forecast-behavior; prediction-core-parity; model-truth (manager disabled).",
    governanceNotes: "candidate in sources.ts; model-truth.ts marks it disabled-pending-backtest (active:false). Binary, uncapped; production weight (MODEL_WEIGHTS.manager) is currently ZERO, so it does not affect probabilities today (no longer a weighted driver).",
  },
  {
