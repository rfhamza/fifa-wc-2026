...
## Status (Phase 1.18C-1; WC-2018 pack added in 1.18B-4)
Contract + **two ingested packs (WC-2022, WC-2018)** + the match-level evaluator. Still **no
tournament replay, no calibration, no weight tuning, no production/probability change.**
- `data/historical/snapshots/wc-2018.ts` — derived WC-2018 snapshot (generated by
  `scripts/generate-historical-2018.mjs`; raw not committed). See `docs/BACKTESTING_WC2018_SNAPSHOT.md`.
- The validator host check is parameterized via `HistoricalPackExpectations`
  (`WC2022_EXPECTATIONS` Qatar/AFC, `WC2018_EXPECTATIONS` Russia/UEFA). Each tournament has a
  dedicated generator; a shared generator is deferred until 2014 (N=3).
